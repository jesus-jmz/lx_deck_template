<?php
  ...
  if(isset($_POST["submit"])){
    ...
    if($act3 == 0){
      $progress = addProgress("metrics", "progress", "deck_id", $deckId, $progress, 4);
      ...
      $_SESSION["act_3"] = $act3;
      $currentSlab = finishSlab($currentSlab, $deckId);
    }
  ...
  $num_instrucciones = count($act3_instructions[$slabNum-1]["instruction_list"]);
  //echo "<br>Número de instrucciones: ".$num_instrucciones;
  $instruction_list = $act3_instructions[$slabNum-1]["instruction_list"];
  $instruction_list = $act3_instructions[$slabNum-1]["instruction_list"];
  ...
  <body>
    ...
    <main class="container w-75">
      ...
      <div class="instrucciones">
        ...
        <?php if(isset($act3_instructions[$slabNum-1]["introduction"])){?>
          <p><?php echo $act3_instructions[$slabNum-1]["introduction"];?></p>
        <?php }?>
        ...
        <ol class="instructions">
          ...
          <?php for($i = 0; $i < $num_instrucciones; $i++){?>
            ...
            <li class="<?php
              if($act3_instructions[$slabNum-1]["type"] == "letter") { echo "letter-instructions "; } 
              if($act3_instructions[$slabNum-1]["text"] == "bold") { echo "bold "; }
              ?> 
        ...
        <?php if(isset($act3_instructions[$slabNum-1]["extra_element"])){?>
          <p><?php echo $act3_instructions[$slabNum-1]["extra_element"]["extra_intro"];?></p>
          <ol>
            <?php for($i = 0; $i< count($act3_instructions[$slabNum-1]["extra_element"]["extra_instruction_list"]);$i++) { ?>
              <li><?php echo $act3_instructions[$slabNum-1]["extra_element"]["extra_instruction_list"][$i]["extra_instruction"]?></li>
            <?php }?>
        <?php }?>
          </ol>
          <!--Finalización-->
          <div class="container mt-4">
          <?php if($act3_finalization[$slabNum-1]["type"]=="row"){?>
            <!--Finalización por filas-->
            <div class="botones-row d-flex flex-column mx-auto">
              <a href="<?= $act3_finalization[$slabNum-1]["tool"]?>" target="_blank" class="tracker btn btn-primary mb-3">Herramienta <i class="bi bi-box-arrow-up-right"></i></a>
          ...
          <?php } else {?>
            ...
            <div class="row">
              <div class="col-md text-center">
                <?php if(isset($act3_finalization[$slabNum-1]["instruction"])){?>
                  <p class="f-instruction"><?= $act3_finalization[$slabNum-1]["instruction"]?></p>
                <?php }?>
                <?php if(isset($act3_finalization[$slabNum-1]["img"])){?>
                  <a id="diana-link" href="https://www.geogebra.org/m/mqsarxyq" target="_blank">
                    <img class="tracker tool-img" src="<?= $act3_finalization[$slabNum-1]["img"]?>" alt="Diana" />
                  </a>
                <?php }?>
              </div>
            ...
            <div class="modal-dialog final-modal-dialog">
              <div class="modal-content">
                <div class="modal-body final-modal-body">
                  ...
                  <p class="modal-message white-text"><?= $act3_final_message[$slabNum-1]?></p>
                  <!--Rating-->
                  <h3 class="subtitle white-text star-text mt-2">Valóranos</h3>
                    ...
                    <!--Botones-->
                  <?php if($currentSlab == 10 && $slabNum == 10) {?>
                    <form class="mt-3" action="autoev.php" method="POST">
                      <input value="Diagnóstico" type="submit" name="submit" class="btn btn-primary w-100"></input>
                    </form>
                  <?php } else {?>
                    <div class="botones-modal d-flex flex-column mx-auto">
                      ...
                      <form class="mt-3" action="autoev.php" method="POST">
                        ...
                        <input value="Slab <?= ($slabNum+1);?>" type="submit" name="submit" class="btn btn-primary w-100"></input>
                      </form>
                    </div>
                  <?php }?>
                </div>
